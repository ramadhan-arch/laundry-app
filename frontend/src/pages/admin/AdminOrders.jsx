...
const statusList = ['pending','processing','washing','drying','ironing','done','delivered','cancelled'];
...
const statusColor = { pending:'badge-pending', processing:'badge-processing', washing:'badge-washing', drying:'badge-drying', ironing:'badge-ironing', done:'badge-done', delivered:'badge-delivered', cancelled:'badge-cancelled' };
...
export default function AdminOrders() {
  ...
  useEffect(() => {
    fetchOrders();
    fetchServices();
    // customers & employees tidak dicache karena jarang berubah tapi perlu fresh
  ...
  return (
    <AdminLayout title="Order Laundry" subtitle="Kelola semua transaksi laundry"
      ...
      <div style={{ background:'#13131a', border:'0.5px solid rgba(255,255,255,0.06)', borderRadius:14, overflow:'hidden' }}>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead>
              <tr style={{ borderBottom:'0.5px solid rgba(255,255,255,0.06)' }}>
                {['Kode','Pelanggan','Layanan','Total','Status','Bayar','Aksi'].map(h => (
            ...
            <tbody>
              {filtered.map(o => (
                <tr key={o.id} style={{ borderBottom:'0.5px solid rgba(255,255,255,0.04)' }}>
                  <td style={{ padding:'12px 14px', fontSize:12, color:'#6C9CF8', fontWeight:500 }}>{o.order_code}</td>
                  ...
                  <td style={{ padding:'12px 14px', fontSize:12, color:'rgba(255,255,255,0.5)', maxWidth:150 }}>
                    {o.OrderItems?.map(i=>i.Service?.name).join(', ')}
                  </td>
                  <td style={{ padding:'12px 14px', fontSize:13, color:'white', fontWeight:500 }}>Rp {Number(o.total_amount).toLocaleString('id-ID')}</td>
                  <td style={{ padding:'12px 14px' }}><span className={`badge ${statusColor[o.status]}`}>{statusLabel[o.status]}</span></td>
                  <td style={{ padding:'12px 14px' }}><span className={`badge badge-${o.payment_status}`}>{o.payment_status==='paid'?'Lunas':'Belum'}</span></td>
                  <td style={{ padding:'12px 14px' }}>
                    <div style={{ display:'flex', gap:6 }}>
                      ...
                      <button onClick={async () => { if(window.confirm('Hapus order?')) { await deleteOrder(o.id); invalidateOrders(); fetchOrders(true); } }}
        ...
        {!loading && !filtered.length && <div style={{ textAlign:'center', padding:40, color:'rgba(255,255,255,0.25)', fontSize:13 }}>Belum ada order</div>}
      </div>
      ...
      {statusModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:200 }}
          ...
          <div style={{ background:'#13131a', borderRadius:14, padding:24, width:380, border:'0.5px solid rgba(255,255,255,0.1)' }} onClick={e => e.stopPropagation()}>
            <h3 style={{ color:'white', fontSize:15, fontWeight:500, marginBottom:4 }}>Update Status</h3>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:20 }}>
              {statusList.map(s => (
                <button key={s} onClick={() => setStatusModal({...statusModal,next:s})}
                  style={{ padding:'7px 14px', borderRadius:8, border:'0.5px solid', borderColor:statusModal.next===s?'#6C9CF8':'rgba(255,255,255,0.1)', background:statusModal.next===s?'rgba(108,156,248,0.15)':'transparent', color:statusModal.next===s?'#6C9CF8':'rgba(255,255,255,0.45)', fontSize:12 }}>
                  {statusLabel[s]}
                </button>
              ))}
            </div>
            <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
              <button onClick={() => setStatusModal(null)} style={{ padding:'8px 16px', borderRadius:8, border:'0.5px solid rgba(255,255,255,0.1)', background:'transparent', color:'rgba(255,255,255,0.5)', fontSize:13 }}>Batal</button>
              <button onClick={handleStatus} style={{ padding:'8px 18px', borderRadius:8, border:'none', background:'#6C9CF8', color:'white', fontSize:13 }}>Simpan</button>
            </div>
          </div>
        </div>
      )}
      ...
      {payModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:200 }}
          ...
          <div style={{ background:'#13131a', borderRadius:14, padding:24, width:360, border:'0.5px solid rgba(255,255,255,0.1)' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ background:'rgba(29,158,117,0.1)', borderRadius:10, padding:'12px 16px', marginBottom:16, textAlign:'center' }}>
            ...
            <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
              <button onClick={() => setPayModal(null)} style={{ padding:'8px 16px', borderRadius:8, border:'0.5px solid rgba(255,255,255,0.1)', background:'transparent', color:'rgba(255,255,255,0.5)', fontSize:13 }}>Batal</button>
              <button onClick={handlePay} style={{ padding:'8px 18px', borderRadius:8, border:'none', background:'#1D9E75', color:'white', fontSize:13 }}>Konfirmasi Lunas</button>
            </div>
          </div>
        </div>
      )}
